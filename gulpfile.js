...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/asp.netmvc-mobile-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/asp.netmvc-mobile-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore') {
                ...
                if (fs.existsSync('../asp.netmvc-mobile-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/asp.netmvc-mobile-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../asp.netmvc-mobile-docs/` + changedFileName, `../../../gitlabRepo/asp.netmvc-mobile-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/asp.netmvc-mobile-docs/')) {
                            shelljs.cp('-rf', `../asp.netmvc-mobile-docs/` + changedFileName, `../../../gitlabRepo/asp.netmvc-mobile-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/asp.netmvc-mobile-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/asp.netmvc-mobile-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/asp.netmvc-mobile-docs`);
